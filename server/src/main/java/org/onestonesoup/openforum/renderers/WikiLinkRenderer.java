...
public class WikiLinkRenderer implements WikiTagRenderer {
	...
	public void render(String pageName, StringBuffer target, StringBuffer data,
			int count, OpenForumController controller, WikiLinkParser linkParser) {
			...
			if (link.external == true) {
				if (isImage(link.href)) {
					...
					target.append("<a href=\""
							...
							+ "\" onclick=\"window.open('"
							+ link.href
							+ "');return false;\"><img src=\"/OpenForum/Images/icons/gif/link_go.gif\" border=\"0\" class=\"node\"></a>");
				} else {
					target.append("<a href=\""
							+ link.href
							+ "\" title=\""
							...
							+ "');return false;\">"
							+ link.displayName
							+ " <img src=\"/OpenForum/Images/icons/gif/link_go.gif\" border=\"0\" class=\"node\"></a>");
				}
			} else {
				if (link.href.indexOf("/OpenForum/Actions/Edit?") != -1) {
					String missingPageName = link.href.substring(33);
					target.append("<a href=\"" + link.href
							+ "\" title=\"The page " + missingPageName
							+ " does not exist. Click to create this page.\">"
							+ link.displayName + "</a>");
				} else {
					target.append("<a href=\"" + link.href + "\" title=\""
							+ link.href + "\">" + link.displayName + "</a>");
				}
			}
		} catch (Exception e) {
			target.append("[" + data + "]");
