...
public class WikiLinkParser {
	...
	public WikiLinkParser(String pageName, String editPageLink,
			String editPageTemplate, OpenForumController controller)
	...
	public WikiLink parseLink(OpenForumController controller, String data)
			throws Exception, AuthenticationException {
		String href = data;
			...
			if (getWikiLink(href) != null) {
				...
				if (wikiHref.indexOf('#') > -1) {
					...
					if (wikiHref.length() == 0) {
						href = "";
						return new WikiLink(displayName, href, anchor,
								parameters, external);
					}
				}
				...
				/*
				 * if(wikiHref.charAt(0)=='/') { wikiHref =
				 * wikiHref.substring(1); }
				 */
				wikiHref = OpenForumNameHelper.titleToWikiName(wikiHref);

				if (getWikiLink(wikiHref) == null) {
					if (wikiHref.indexOf('/') != -1) {
						String pageName = wikiHref.substring(0,
								wikiHref.lastIndexOf('/'));
						String fileName = wikiHref.substring(wikiHref
								.lastIndexOf('/'));
						if (controller.getFileManager()
								.pageAttachmentExists(pageName, fileName,
										controller.getSystemLogin())) {
							displayName = wikiHref;
							href = wikiHref;

							WikiLink link = new WikiLink(displayName, href,
									anchor, parameters, false);
							return link;
						}
					}

					String valid = OpenForumNameHelper
							.validateWikiTitle(wikiHref);

					if (valid.equals("OK") == false) {
						valid = valid.replace("\"", "&quot;");
						href = "javascript:ui.showAlert('Invalid Wiki Page Name','A Wiki page name "
								+ valid + "');";
						...
						List<KeyValuePair> dynamicPages = controller
								.getDynamicPagesList();
						if (dynamicPages != null) {
							for (int loop = 0; loop < dynamicPages.size(); loop++) {
								KeyValuePair dynamicPage = dynamicPages
										.get(loop);
		...
		WikiLink link = new WikiLink(displayName, href, anchor, parameters,
				external);

		return link;
	}

	public Map<String, String> getAttachmentsForPage(
			OpenForumController controller) throws Exception,
			AuthenticationException {
		Map<String, String> links = controller.getFileManager()
				.getPageAttachments(pageName, controller.getSystemLogin());

		return links;
	}

	private String getWikiLink(String wikiHref) throws AuthenticationException,
			Exception {
		String link = null;
		...
		if (controller.getFileManager().attachmentExists(wikiHref, PAGE_FILE,
				controller.getSystemLogin())) {
			link = OpenForumNameHelper.titleToWikiName(wikiHref);
		} else {
			try {
				if (controller.getFileManager().attachmentExists(wikiHref,
						controller.getSystemLogin())) {
					return wikiHref;
				}
			} catch (Exception e) {
