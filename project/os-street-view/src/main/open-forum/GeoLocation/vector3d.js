...
Vector3d.prototype.angleTo = function(v, vSign) {
    var sinTh = this.cross(v).length();
    var cosTh = this.dot(v);

    if (typeof vSign != 'undefined') {
        // use vSign as reference to get sign of sinTh
        sinTh = this.cross(v).dot(vSign)<0 ? -sinTh : sinTh;
    }

    return Math.atan2(sinTh, cosTh);
};
