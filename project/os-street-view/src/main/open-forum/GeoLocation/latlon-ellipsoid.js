...
LatLonE.prototype.toCartesian = function() {
    var om = this.lat.toRadians(), lm = this.lon.toRadians();
    var h = this.height; // above ellipsoid
    var a = this.datum.ellipsoid.a, b = this.datum.ellipsoid.b;

    var sinom = Math.sin(om), cosom = Math.cos(om);
    var sinlm = Math.sin(lm), coslm = Math.cos(lm);

    var eSq = (a*a - b*b) / (a*a);
    var v = a / Math.sqrt(1 - eSq*sinom*sinom);

    var x = (v+h) * cosom * coslm;
    var y = (v+h) * cosom * sinlm;
    var z = ((1-eSq)*v + h) * sinom;
...
Vector3d.prototype.toLatLon = function(datum) {
    ...
    var ep2 = (a*a-b*b) / (b*b); // 2nd eccentricity squared
    var p = Math.sqrt(x*x + y*y);
    var R = Math.sqrt(p*p + z*z); // radius

    // u = parametric latitude (Bowring eqn 17)
    var tanu = (b*z)/(a*p) * (1+ep2*b/R);
    var sinu = tanu / Math.sqrt(1+tanu*tanu);
    var cosu = sinu / tanu;

    // geodetic latitude (Bowring eqn 18)
    var B = Math.atan2(z + ep2*b*sinu*sinu*sinu,
                       p - e2*a*cosu*cosu*cosu);

    // longitude
    var lm = Math.atan2(y, x);

    // height above ellipsoid (Bowring eqn 7)
    var sinB = Math.sin(B), cosB = Math.cos(B);
    var v = a*Math.sqrt(1-e2*sinB*sinB); // length of the normal terminated by the minor axis
    var h = p*cosB + z*sinB - (a*a/v);

    var point = new LatLonE(B.toDegrees(), lm.toDegrees(), datum, h);
