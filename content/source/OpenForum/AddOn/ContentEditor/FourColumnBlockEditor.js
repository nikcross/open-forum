...
* Description: 
*/
var FourColumnBlockEditor = function(data, parentId) {
  var self = this;
  self.setData( data );
  ...
  self.initView = function( elementId ) {
    ...
    var view = "<div class='columns large-3'>";
    ...
    view += "<div class='columns large-3'>";
    ...
    view += "<div class='columns large-3'>";
    view += self.renderAddButton( elementId, "addColumnD", false);
    for( var i in data.content.columnD ) {
      data.content.columnD[i].elementId = ("BK" + Math.random() + new Date().getTime()).replaceAll("\\.","_");
      view += "<span id='" + data.content.columnD[i].elementId + "'><div style='border: solid 1px; height: 150px;'>Loading...</div></span>";
      view += self.renderAddButton( data.content.columnD[i].elementId, "addAfter", true);
    }
    view += "</div>";
  ...
  var getContentIndex = function(id) {
    ...
    if(index==-1) {
      row = "columnD";
      for(var i in data.content.columnD) {
        if(data.content.columnD[i].elementId == id) {
          index = i;
          break;
        }
      }
  ...
  self.addBlock = function(instruction,type) {
    ...
    } else  if(instruction.action=="addColumnD") {
      var template = OpenForum.evaluate( type+"BlockEditor.template" );
      var json = OpenForum.clone(template);

      data.content.columnD.splice( 0,0,json );
    }
  ...
  var initBlocks = function() {
    ...
    for( var i in data.content.columnC ) {
      ...
      (function(index) {
        OpenForum.waitFor(
          function() {
            return document.getElementById(data.content.columnD[index].elementId) != null;
          },
          function() {
            createBlockEditor( data.content.columnD[index].block, data.content.columnD[index], data.elementId, data.content.columnD[index].elementId, data.treeNode );
          },
          500
        );
      } )(i);
  ...
  self.render = function() {
    ...
    content += "</div>";
        content += "<div class='columns large-3'>";
    for( var i in data.content.columnD ) {
      content += data.content.columnD[i].blockEditor.render();
    }
    content += "</div>";
    content += "</div>";
    return content;
...
FourColumnBlockEditor.prototype = new BlockPrototype("Four Column", "text_columns");

FourColumnBlockEditor.template = {
  "pageName": "",
  "block": "fourColumn",
  content: {
    columnA: [],
