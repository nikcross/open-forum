...
* Description: v0.0.3
*/
var CTABannerBlockEditor = function(data, parentId) {
  var self = this;
  self.setData( data );
  ...
  self.initView = function(elementId) { 
    ...
    OpenForum.getObject( elementId + "_ctaText" ).setValue( data.data.ctaText );
    OpenForum.addListener( elementId + "_ctaText", function( obj ) { data.data.ctaText=obj.getValue(); } );

    OpenForum.getObject( elementId + "_ctaURL" ).setValue( data.data.ctaURL );
    OpenForum.addListener( elementId + "_ctaURL", function( obj ) { data.data.ctaURL=obj.getValue(); } );
  ...
  view += "ctaText: <input type='text' of-id='" + elementId + "_ctaText" + "' value='" +data.data.ctaText + "'/>";

  view += "ctaURL: <input type='text' of-id='" + elementId + "_ctaURL" + "' value='" +data.data.ctaURL + "'/>";
    ...
    data.treeNode.setName( self.renderHighlightLink( elementId + "_content", data.treeNode.getName(), "CTABanner" ) );
...
self.render = function() {
  var content = "[{/OpenForum/Extensions/CTABanner imageURL='" + data.data['imageURL'] + "' title='" + data.data['title'] + "' text='" + data.data['text'] + "' ctaText='" + data.data['ctaText'] + "' ctaURL='" + data.data['ctaURL'] + "' }]";

  return content;
};
};

CTABannerBlockEditor.prototype = new BlockPrototype("CTABanner","{{blockIcon}}");

CTABannerBlockEditor.template = {
  "block": "cTABanner",
  "data": {
