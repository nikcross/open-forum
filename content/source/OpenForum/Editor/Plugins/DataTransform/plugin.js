...
if( typeof dataTransform === "undefined" ) {
  ...
  addPlugin( {
    ...
    init: function() {
      ...
      editorList[editorIndex] = {id: editorIndex, tabButtonStyle: "tab", tabId: "editor"+editorIndex, name: "Data Transform", changed: "", options: [], plugin: dataTransform};
      showTab(editorIndex);
  ...
  dataTransform.load = function(iPageName,fileName) {
    if(!fileName) {
      fileName = iPageName;
      iPageName = pageName;
    }
    var data = OpenForum.loadFile( iPageName + "/" + fileName);
    dataTransform.input.setValue( data );
  };
  ...
  dataTransform.save= function(iPageName,fileName) {
    if(!fileName) {
      fileName = iPageName;
      iPageName = pageName;
    }
    OpenForum.saveFile( iPageName + "/" + fileName, output , function() { alert("Saved to " + iPageName + "/"+ fileName ); }, function() {alert("Saved file "+iPageName + "/" + fileName,"Data Transform");});
  };
  ...
  dataTransform.openFileSelect = function() {
    if(typeof(dataTransform.tree)=="undefined") {
      ...
      if(pageName!=="/OpenForum/Editor/Plugins/DataTransform") {
        dataTransform.localTree = new Tree("transformLocalFileTree","Loading...","");
        JSON.get("/OpenForum/Javascript/Tree","getPageTree","pageName="+pageName).onSuccess(
          function(result) {
            dataTransform.modifyTreeNode(result);
            dataTransform.localTree.setJSON(result);
            dataTransform.localTree.render();
            dataTransform.localTree.getRoot().expand();
            dataTransform.localTree.init();
          }
        ).go();
      }
    }
  ...
  dataTransform.importTreeNode = function(node,url,action,parameters) {
    JSON.get(url,action,parameters).onSuccess(
      function(response) {
        for(var i in response.leaves) {
          dataTransform.modifyTreeNode( response.leaves[i] );
          node.addJSON( response.leaves[i] );
        }
        node.paint();
      }
    ).go();
  ...
  dataTransform.modifyTreeNode = function(data) {
    ...
    if(data.attributes.actions) {
      ...
      for(var a = 0;a<actions.length;a++) {
        ...
        } else if(data.attributes.type==="more" && actions[a].icon==="add") { 
          //function(node){ node.getParent().importJSON( '/OpenForum/Javascript/Tree','getPageTree','pageName=' + node.getAttribute('pageName') + '&match=.*&fileOffset=22&pageOffset=0' );node.getParent().deleteChild(node);}
          actions[a].fn = actions[a].fn.replace("node.getParent().importJSON(","dataTransform.importTreeNode(node.getParent(),");
          newActions.push( actions[a] );
        }
      }
  ...
  dataTransform.saveLocalTransformFile = function() {
    OpenForum.saveFile(pageName+"/"+dataTransform.transformFileName, dataTransform.process.getValue(), function() {alert("Saved file "+pageName+"/"+dataTransform.transformFileName,"Data Transform");});
  };

  dataTransform.saveGlobalTransformFile = function() {
    OpenForum.saveFile("/OpenForum/Editor/Plugins/DataTransform/"+dataTransform.transformFileName, dataTransform.process.getValue(), function() {alert("Saved file /OpenForum/Editor/Plugins/DataTransform/"+dataTransform.transformFileName,"Data Transform");});
  };
  ...
  for(var i in editorList) {
    if(editorList[i].name==="Data Transform") {
