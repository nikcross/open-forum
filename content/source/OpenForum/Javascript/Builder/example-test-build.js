//==============================================================================================================//
/* Version 1.0.0*/
/* Built on Wed Nov 18 2015 10:24:55 GMT-0000 (UTC) */
...
var OpenForum = new function(){
  var FIELD_DELIMETER_START = "{"+"{";
  var FIELD_DELIMETER_END = "}"+"}";
  var self = this;
  var objects= [];
  var tables = [];
  var tabs = [];
  ...
  self.getBuildDate = function() {
    return "Wed Nov 18 2015 10:24:55 GMT-0000 (UTC)";
  };
  ...
  self.crawlParts = function (node,prefix) {
    ...
    if(!(node.childNodes && node.childNodes.length>0)) {          
      ...
      if( typeof(node.innerHTML)!="undefined" && node.innerHTML.indexOf(FIELD_DELIMETER_START)!=-1) {
        self.parseParts(node,objects,prefix);
      }
      if( node.nodeName=="#text" && node.nodeValue.indexOf(FIELD_DELIMETER_START)!=-1) {
  ...
  self.parseText= function(node,objects,prefix) {
    ...
    var spans = [];
    while(data.indexOf(FIELD_DELIMETER_START)!=-1) {
      name = data.substring(data.indexOf(FIELD_DELIMETER_START)+2,data.indexOf(FIELD_DELIMETER_END));

      data = data.substring(0,data.indexOf(FIELD_DELIMETER_START))+
        "<span id='OpenForumId"+nextId+"'>OpenForumId"+nextId+"</span>"+
        data.substring(data.indexOf("FIELD_DELIMETER_END")+2);
