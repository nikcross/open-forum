...
function DefaultRenderer() {
  ...
  self.getVersion = function() {
    return "DefaultRenderer v1.07 The Jester";
  };

  var search = function(value,renderer) {
    var match = renderer.getStart();
    if(renderer.isRegEx()) {
      return value.search(match);
    } else {
  ...
  var MarkUp = function(config) {
    ...
    self.isRegEx = function() {
      return !(!match.regex);
    };

    self.getStartLength = function(data) { 
      if(self.isRegEx()) {
        return data.search(match.start) + match.length;
      } else {
  ...
  var renderRadioButton = function (pageName,content) {
    if( content.indexOf(":")==0 ) {
      ...
      return "<input type='radio' id='"+id+"' value='"+id+"' of-id='"+name+"' name='"+name+"' />";
    } else {
  ...
  var renderLink = function (pageName,content) {
    ...
    if(link.indexOf(".png")!=-1) {
      ...
      //create link
      if(newTab) {
        output = "<a href=\""+link+"\""+target+" target=\"newTab\">"+title+"</a>";
      } else {
  ...
  var findFirstMatch = function(source,matchers) {
    ...
    for(var i=0; i<matchers.length; i++) {
      ...
      var point  = search(source, renderer );
      if (point >= 0 && point < lowestIndex) {
        lowestIndex = search(source, renderer );
  ...
  /*===================================*/
  var markUp = [
    new MarkUp( { type: "list", match: {start: SOL+"\\*", regex: true, length: 2, end: EOL}, start: "<li>", end: "</li>", open: "<ul>", close: "</ul>" } ),
  ...
  self.render = function(pageName,text) {
    try{
      ...
      var MAX_OUTPUT = 100;
      var MAX_RENDER_STACK = 10;

      //While there is input text left to render
      while(input.length>0) {
        count++;
        if(count>MAX_ITERATIONS) throw "Page " + pageName + " exceeded maximum iterations. Render quit.";
        if(output.length>MAX_OUTPUT) throw "Page " + pageName + " exceeded maximum output. Render quit.";
        if(rendererStack.length>MAX_RENDER_STACK) throw "Page " + pageName + " exceeded maximum output. Render quit.";
        ...
        if(renderer===null) {
          ...
          var startPoint = search(input, renderer );
          ...
          var after = input.substring(startPoint+renderer.getStartLength(input));
...
log = {
  error: function(message) {
    //localConsole.log("Error: "+message);
	}
};

renderer = new DefaultRenderer();

console.log( renderer.render("TestPage","*A \n# B \n# C \n\nsome trailing text") );
