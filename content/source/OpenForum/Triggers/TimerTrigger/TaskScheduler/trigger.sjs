...
try{
  ...
  for(var i=0;i<schedule.length;i++) {
    ...
    if(task.scheduledTime==="Every ten seconds") {
      ...
      if(now.getDate()==1 && now.getHour()==8) {
        run(task);
      }
    } else if(trigger.isHourPeriod() && task.scheduledTime.startsWith("Every week")) {
      var day = 1;
      if(task.scheduledTime.toLowerCase().contains(" on ")) {
      ...
      if(now.getDay()==day && now.getHour()==8) {
        run(task);
      }
    } else if(trigger.isHourPeriod() && task.scheduledTime==="Every year") {
      if(now.getMonth()===0 && now.getDate()==1 && now.getHour()==8) {
        run(task);
      }
      ...
      var testDate;
      if(task.scheduledTime.contains(" at ")) {
        ...
        testDate = new Date().parseDDMMYYYY(task.scheduledTime.toLowerCase().between("on "," "));
        testDate.setHHMM("08:00");
        if( dateMatches(now,testDate) && timeMatches(now,testDate) ) {
          run(task);
        }
      }
